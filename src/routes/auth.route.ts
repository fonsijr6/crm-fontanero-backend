...
router.post('/refresh', async (req, res) => {
  try {
    ...
    const payload  = typeof verified === 'string' ? { sub: verified } : (verified as JwtPayload);

    const sub = String(payload.sub ?? '');
    if (!sub) return res.status(401).json({ error: 'INVALID_REFRESH' });

    const accessOptions: SignOptions = { expiresIn: ACCESS_TTL };
